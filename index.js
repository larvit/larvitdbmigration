...
function DbMigration(options) {
	...
	if	(options.tableName === undefined && options.indexName !== undefined) options.tableName = options.indexName;
	else if	(options.indexName === undefined && options.tableName !== undefined) options.indexName = options.tableName;

	if (options.tableName	=== undefined) options.tableName = options.indexName	= 'db_version';
	if (options.migrationScriptsPath	=== undefined) options.migrationScriptsPath	= './dbmigration';

	log.verbose(logPrefix + 'Started with options: ' + JSON.stringify(options));
...
DbMigration.prototype.getLock = function getLock(cb) {
	const	logPrefix	= topLogPrefix + 'DbMigration.prototype.getLock() - ';
			...
			dbCon.query('SELECT running FROM `' + options.tableName + '`', function (err, rows) {
				...
				if (rows[0].running === 0) {
					...
					dbCon.query('UNLOCK TABLES;', function (err) {
						...
						log.info(logPrefix + 'Another process is running the migrations for table ' + options.tableName + ', wait and try again soon.');
						setTimeout(function () {
							getLock(cb);
						}, 500);
					});
...
DbMigration.prototype.run = function run(cb) {
	const	logPrefix	= topLogPrefix + 'DbMigration.prototype.run() - ';
		...
		return function (cb) {
			...
			function runScripts(startVersion, cb) {
				log.verbose(logPrefix + 'runScripts() - Started with startVersion: "' + startVersion + '" in path: "' + options.migrationScriptsPath + '" for table ' + options.tableName);
				...
				try {
					fs.readdir(options.migrationScriptsPath, function (err, items) {
						...
						if (err) {
							log.info(logPrefix + 'runScripts() - Could not read migration script path "' + options.migrationScriptsPath + '"');
							cb();
							return;
						}
						...
						for (let i = 0; items[i] !== undefined; i ++) {
							if (items[i] === startVersion + '.js') {
								log.info(logPrefix + 'runScripts() - Found js migration script #' + startVersion + ' for table ' + options.tableName + ', running it now.');
								require(options.migrationScriptsPath + '/' + startVersion + '.js')(function (err) {
									if (err) {
										log.error(logPrefix + 'runScripts() - Got error running migration script ' + options.migrationScriptsPath + '/' + startVersion + '.js' + ': ' + err.message);
										cb(err);
										return;
									}

									log.debug(logPrefix + 'runScripts() - Js migration script #' + startVersion + ' for table ' + options.tableName + ' ran. Updating database version and moving on.');
								...
								log.info(logPrefix + 'runScripts() - Found sql migration script #' + startVersion + ' for table ' + options.tableName + ', running it now.');
								...
								dbCon.query(fs.readFileSync(options.migrationScriptsPath + '/' + items[i]).toString(), function (err) {
									...
									log.info(logPrefix + 'runScripts() - Sql migration script #' + startVersion + ' for table ' + options.tableName + ' ran. Updating database version and moving on.');
									db.query(sql, function (err) {
										if (err) { cb(err); return; }
						...
						log.info(logPrefix + 'runScripts() - Database migrated and done. Final version is ' + (startVersion - 1) + ' in table ' + options.tableName);

						// If we end up here, it means there are no more migration scripts to run
						cb();
					});
				} catch(err) {
					log.error(logPrefix + 'runScripts() - Uncaught error: ' + err.message);
					cb(err);
				}
			}
			...
			tasks.push(function (cb) {
				db.query('SELECT version FROM `' + options.tableName + '`;', function (err, rows) {
					...
					log.info(logPrefix + 'Current database version for table ' + options.tableName + ' is ' + curVer);
